...
\title{Konzeptpapier\\Explosion Guy}
...
\begin{abstract}
	Die Online-Spiel-Branche erfreut sich coronabedingt immer größerer Beliebtheit. Dies könnte vor allem aus der unkomplizierten Zugangsmöglichkeit resultieren, aber auch der sich als verzichtbar erweisenden Softwareinstallation. Damals wurden diese Art von Spielen mit Adobes Software 'Flash' realisiert.
	
Aufgrund der Risiken auf der Sicherheitsebene wurde die Unterstützung der Software eingestellt. Heutzutage bedient man sich verschiedener Webtechnologien, die im Kapitel IV näher beleuchtet werden.
	
Darin begründet liegt die Motivation hinter dieser Arbeit, denn die Spieleentwicklung ermöglicht eine intensive Auseinandersetzung mit diesen Webtechnologien, aber auch der Kreativität seinen freien Lauf zu lassen.
...
Bomber-Mouse \cite{bombermouse} hingegen ist eine simplere Version, in der sich der Charakter nur sehr langsam fortbewegt, was möglicherweise nicht beabsichtigt ist. Auch die Fenstergröße passt sich nicht automatisch an und das Spiel macht allgemein einen unfertigen Eindruck. Lediglich die Grafik ist im Vergleich zu den Gameplay-Elementen aufwändigergestaltet.
\begin{figure}[H]
    \centering
...
Als Spieler möchte ich, dass die Bomben explodieren und verschwinden, um ihren Effekt visuell zu erleben.
...
Als Spieler möchte ich bei hohen Spielerzahlen keine Wartezeit haben, um jederzeit spielen zu können.
...
Als Spieler möchte ich eine dynamische Spielfeldgröße, um den Umfang des Spiels zu erhöhen.
...
Als Spieler möchte ich eine Rangliste einsehen können, um meine Leistung kompetitiv einordnen zu können.
...
\subsection{Anforderung 19}
Als Spieler möchte ich visualisiert bekommen, wer das Spiel gewonnen hat.

\section{Methoden}
Die Spieldaten werden in einer Datenbank gespeichert.
Diese beinhaltet Informationen zu den Spieleinstellungen, Spielerprofilen und Spielstatistiken. 
Die Verwaltung der Spieldaten und Logik soll im Hintergrund über einen oder mehrere Amazon S3 Buckets geregelt werden, um dadurch eine Skalierbarkeit zu ermöglichen. Werden weitere Kapazitäten für ein neues Spiel benötigt, so kann beispielsweise dafür ein neuer Bucket initialisiert werden. 
Die Umgebung innerhalb des Servers wird mit NodeJS umgesetzt werden, da dies das Bereitstellen der Assets für die User ermöglicht. Um den Spielstand zwischen den Spielern zu synchronisieren, sollen diese eine Verbindung über Websockets mit dem Spielserver aufbauen.
Die Spieloberfläche, die der Nutzer zu sehen bekommt, wird mit HTML5 und JavaScript implementiert. Des Weiteren wird das Framework 'Phaser' verwendet, um dem Nutzer grafische Ausgaben zu liefern und die Eingaben des Nutzers an das Spiel zu verarbeiten.
Da \glqq Cheating\grqq{} eine große Problematik, insbesondere beim Multiplayer-Spiel darstellt, werden alle empfangenen Daten auf dem Server validiert. Dies löst zwar das Problem nicht vollständig, sollte jedoch kleine Angriffe abwehren. 
\begin{figure}[H]
    \centering
